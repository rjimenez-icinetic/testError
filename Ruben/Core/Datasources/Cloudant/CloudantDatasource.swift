...
class CloudantDatasource <T: CloudantItem> {
    ...
    let filter = CloudantDatasourceFilter()
    ...
    func query(datasourceOptions: DatasourceOptions?) -> [String: AnyObject] {
        ...
        if cloudantManager.searchIndexesCreated {
            
            if let searchText = datasourceOptions.searchText where !searchText.isEmpty {
                query["$text"] = ["$search": "\(searchText)*"]
            }
        }
        
        if cloudantManager.filterIndexesCreated {
...
extension CloudantDatasource : Datasource {
    ...
    func distinctValues(name: String, filters: [Filter]?, success: (([String]) -> Void), failure: ((NSError?) -> Void)) {
        ...
        success([]) // Distinct query is not supported by Cloudant Sync
    }
}
