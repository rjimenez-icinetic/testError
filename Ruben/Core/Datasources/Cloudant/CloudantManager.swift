...
enum Cloudant {
    ...
    static let filterSearch = "filterSearch"
...
class CloudantManager <T: CloudantItem>: NSObject, CDTReplicatorDelegate {
    ...
    var filterIndexesCreated = false
    ...
    init(url: NSURL?, datastoreName: String?, indexes: [String]? = nil) {
        ...
        if let indexes = indexes, datastore = datastore where indexes.count != 0 {
            
            if let _ = datastore.ensureIndexed(indexes, withName: Cloudant.filterSearch, ofType: CDTQIndexType.JSON) {
                
                filterIndexesCreated = true
            }
            
            if datastore.textSearchEnabled {
                
                if let _ = datastore.ensureIndexed(indexes, withName: Cloudant.indexSearch, ofType: CDTQIndexType.Text) {
