...
class TrousersViewController: TableViewController<ProductsDSItem>, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        AnalyticsManager.sharedInstance?.analytics?.logPage("Trousers")
       
	    navigationItem.title = NSLocalizedString("Trousers", comment: "")
	
        datasourceOptions = DatasourceOptions()
        datasourceOptions.addDesignFilter(datasource?.datasourceFilter.create(ProductsDSItemMapping.category, string: "Trousers"))
			

        tableView.dataSource = self
        tableView.delegate = self
        tableView.registerClass(Cell.self, forCellReuseIdentifier: Cell.identifier)
		behaviors.append(FilterBehavior<TrousersFilterViewController>(viewController: self))
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let destinationViewController = TrousersDetailViewController()
...
extension TrousersViewController: DataResponse {
